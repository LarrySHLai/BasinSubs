...
###### IMPORT SOUTHERN COASTAL RANGE
#for porosity-depth functions
PoFun = read.csv('Backstripping/Pofun.csv')
#for sea level change
deltaSL = read.csv('Sealevel/SL_Unit_S.csv')
#for stratigraphy data
Strat = read.csv('Thickness/Ages_S_new.csv')
#lithology
Litho <- read.csv('Thickness/Litho_S.csv')
#Paleobathymetry
PB <- read.csv('Thickness/PB_Unit_S.csv')
...
#####
#plotting SOUTHERN COASTAL RANGE
pdf("Figures/Subsidence_S.pdf", width=3.5, height=3,pointsize = 9)

#background
plot(Su[2:length(Su)]~age[2:length(age)], type="l", xlab="Age (Ma)", ylab="Depth, Elevation (m)", main="Southern Coastal Range",xlim=rev(c(0,5)), ylim=rev(c(-2000,9000)))
grid(nx = -2:8, ny = NULL, col = "grey", lty = "dotted",lwd = par("lwd"), equilogs = TRUE)
rug(seq(0, 5, by=0.2), ticksize = -0.02, side = 1)
rug(seq(-2000, 9000, by=5000), ticksize = -0.02, side = 2)
...
arrows(c(mean(c(5.53,4.31)),mean(c(3.82,3.35))), c(200-100,0-100), c(mean(c(5.53,4.31)),mean(c(3.82,3.35))), c(200+100,0+100), length=0, angle=0, code=3, lwd=1,col="grey")
arrows(c(mean(c(5.53,4.31))-(5.53-4.31)/2,mean(c(3.82,3.35))-(3.82-3.35)/2), c(200,0), c(mean(c(5.53,4.31))+(5.53-4.31)/2,mean(c(3.82,3.35))+(3.82-3.35)/2), c(200,0), length=0, angle=0, code=3, lwd=1,col="grey")
points(c(200,0)~c(mean(c(5.53,4.31)),mean(c(3.82,3.35))), pch=2, cex=.6,col="grey")
lines(c(mean(c(5.53,4.31)),mean(c(3.82,3.35))),c(200, 0),lty = "dashed",col="#808080")
...
text(0.15, 6500, paste(format(MaxUrate,digits=3) ,"\u00b1",format(MaxUrate_err,digits=2)), col="blue", cex = .6)
text(0.2, 7000, paste("mm/yr"), col="blue", cex = .6)
text(.6, 1700, paste(format(MinUrate,digits=2) ,"\u00b1",format(MinUrate_err,digits=1)), col="#b8baff", cex = .6)
text(.6, 2200, paste("mm/yr"), col="#b8baff", cex = .6)
...
text(2.5, 3700, paste(format(Srate_2,digits=2) ,"\u00b1",format(Srate_2_err,digits=1)), col="blue", cex = .6)
text(2.5, 4200, paste("mm/yr"), col="blue", cex = .6)
...
print(Results)
write.csv(Results, "Subsidence results_S.csv", row.names = FALSE)
...
print(Results2)
write.csv(Results2, "vertical rates_S.csv", row.names = FALSE)
